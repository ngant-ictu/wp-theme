...
function kontruk_scripts() {	
	...
	if ( !is_admin() ){	
		...
		wp_enqueue_style('kontruk-css', $app_css, array(), null);
		...
		if( is_rtl() || $kontruk_direction == 'rtl' ){
			wp_enqueue_style('rtl-css', get_template_directory_uri() . '/css/rtl.css', array(), null);
		}
		wp_enqueue_style('kontruk-responsive', get_template_directory_uri() . '/css/app-responsive.css', array(), null);
		
		/* Load style.css from child theme */
		if (is_child_theme()) {
			wp_enqueue_style('kontruk-child', get_stylesheet_uri(), false, null);
		}
	...
	if ( !is_admin() ){
		...
		$translation_text = array(
			'cart_text' 		 => esc_html__( 'Add To Cart', 'kontruk' ),
			'compare_text' 	 => esc_html__( 'Add To Compare', 'kontruk' ),
			'wishlist_text'  => esc_html__( 'Add To WishList', 'kontruk' ),
	...
	$overflow_text = array(
		'more_text' => esc_html__( 'More...', 'kontruk' ),
	...
	*/
	if( class_exists( 'WooCommerce' ) ) {
		...
		$kontruk_dest_folder = ( function_exists( 'sw_wooswatches_construct' ) ) ? 'woocommerce' : 'woocommerce_select';
		...
		$wc_add_to_cart_variation_params = array(
			'i18n_no_matching_variations_text' => esc_attr__( 'Sorry, no products matched your selection. Please choose a different combination.', 'kontruk' ),
			'i18n_make_a_selection_text'       => esc_attr__( 'Please select some product options before adding this product to your cart.', 'kontruk' ),
			'i18n_unavailable_text'            => esc_attr__( 'Sorry, this product is unavailable. Please choose a different combination.', 'kontruk' ),
		);
		...
		$quickview_text = array(			
			...
			'add_to_cart_variable' => esc_url( get_template_directory_uri() . '/js/'. $kontruk_dest_folder .'/add-to-cart-variation.min.js' ),
	...
	*/
	if( kontruk_mobile_check() ) :
		...
		wp_dequeue_script( 'kontruk-megamenu' );
		...
		wp_enqueue_style('kontruk-mobile', $mobile_css, array(), null);
	endif;
...
add_action('wp_enqueue_scripts', 'kontruk_scripts', 100);