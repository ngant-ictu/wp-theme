<?php
class kontruk_ResmenuSB{
	function __construct(){
		add_filter( 'wp_nav_menu_args' , array( $this , 'kontruk_MenuRes_AdFilter' ), 100 ); 
		add_filter( 'wp_nav_menu_args' , array( $this , 'kontruk_MenuRes_Filter' ), 110 );	
		add_action( 'wp_footer', array( $this, 'kontruk_MenuRes_AdScript' ), 110 );
		add_action( 'wp_footer', array( $this, 'kontruk_responsive_menu_footer' ) );	
	}
	function kontruk_MenuRes_AdScript(){
	...
	function kontruk_MenuRes_AdFilter( $args ){
		$args['container'] = false;
		$menu = get_registered_nav_menus() ;
		foreach( $menu as $kontruk_theme_locate => $menu_description ){
			if( $kontruk_theme_locate == $args['theme_location'] ){
				if( isset( $args['kontruk_resmenu'] ) && $args['kontruk_resmenu'] == true ) {
					return $args;
				}		
				$ResNavMenu = $this->ResNavMenu( $args );
				$args['container'] = '';
				$args['container_class'].= '';	
				$args['menu_class'].= ($args['menu_class'] == '' ? '' : ' ') . 'kontruk-menures';			
	...
	function ResNavMenu( $args ){
		$args['kontruk_resmenu'] = true;		
		$select = wp_nav_menu( $args );
		return $select;
	}
	function kontruk_MenuRes_Filter( $args ){
		/* Fix Menu on wp 4.7 */
		if( !isset( $args['kontruk_resmenu'] ) ){
			return $args;
		}
		$menu = get_registered_nav_menus() ;
	...
	function kontruk_responsive_menu_footer(){
		$menu_locations = swg_options( 'mobile_menu' );
					...
					<div class="menu-responsive-inner">
						...
						<ul class="nav nav-tabs">
							<?php 
								...
								$menu_titles = explode( ',', swg_options( 'mobile_menu_title' ) );
							...
							<?php if( has_nav_menu('primary_menu') ) : ?>
								<li class="<?php echo esc_attr( $class_p ); ?>">
									<a href="#ResPrimary" data-toggle="tab" class="tab-primary"><?php echo esc_html__( 'Menu', 'kontruk' ); ?></a>
								</li>
							<?php endif; ?>
								
							<?php if( has_nav_menu('vertical_menu') ) : ?>
								<li class="<?php echo esc_attr( $class_v ); ?>">
									<a href="#ResVertical" data-toggle="tab" class="tab-vertical"><?php echo esc_html__( 'Categories', 'kontruk' ); ?></a>
								</li>
							<?php endif; ?>
							<?php } ?>
						</ul>
								...
								<div id="<?php echo esc_attr( 'Res'. $location ); ?>" class="tab-pane <?php echo esc_attr( ( $key == 0 ) ? 'active' : '' ); ?>">
									<?php 
										wp_nav_menu( array(
											...
											'walker' => new kontruk_ResMenu_Walker()
										) );
									?>
								</div>
							<?php }
...
class Kontruk_ResMenu_Walker extends Walker_Nav_Menu {
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		if( isset( $item->imgupload ) && !empty( $item->imgupload ) ){
			if( $depth == 0 ){
				$item_html = preg_replace( '/(<a[^>]*>)(.*)(<\/a>)/iU', '$1<span class="menu-img"><img src="'.esc_url( $item->imgupload ).'" alt="'. esc_attr__( 'Thumbnail', 'kontruk' ) .'"/></span><span class="menu-title">$2</span>$3', $item_html );
			}
		}
