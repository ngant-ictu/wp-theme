...
$kontruk_single_thunbmail = ( swg_options( 'product_single_thumbnail' ) && !kontruk_mobile_check() ) ? swg_options( 'product_single_thumbnail' ) : 'bottom';
$post_thumbnail_id	 	  = get_post_thumbnail_id( $post->ID );
?>
<div id="product_img_<?php echo esc_attr( $post->ID ); ?>" class="woocommerce-product-gallery woocommerce-product-gallery--with-images images product-images loading" data-vertical="<?php echo ( ( $kontruk_single_thunbmail == 'left' || $kontruk_single_thunbmail == 'right' ) && !kontruk_mobile_check() ) ? 'true' : 'false'; ?>" data-video="<?php echo sprintf( ( $kontruk_featured_video != '' ) ? '%s' : '', esc_url( 'https://www.youtube.com/embed/' . $kontruk_featured_video ) ); ?>">
	<figure class="woocommerce-product-gallery__wrapper">
	<div class="product-images-container clearfix <?php echo esc_attr( 'thumbnail-' . $kontruk_single_thunbmail ); ?>">
		<?php 
		...
		<?php }else{ ?>			
			<div class="single-img-product">
				<?php echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<img src="%s" alt="%s" />', wc_placeholder_img_src(), esc_attr__( 'Placeholder', 'kontruk' ) ), $post->ID ); ?>
			</div>
		<?php } ?>
	</div>
	</figure>
