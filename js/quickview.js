	...
	**/
	if( $('body').html().match( /sw-quickview-bottom/ ) ){
		...
		$(document).on( 'click', 'a[data-type="quickview"]', function(){
			var product_id = $(this).data( 'product_id' ), ajaxurl = quickview_param.ajax_url.replace( '%%endpoint%%', 'kontruk_quickviewproduct' );
			ajaxurl = ajaxurl.replace( '%endpoint%', 'kontruk_quickviewproduct' );
			qv_target.addClass( 'show loading' );
			var data 		= {
				action: 'kontruk_quickviewproduct',
			...
			jQuery.post(ajaxurl, data, function(response) {
				...
				$( '.quickview-container .product-images' ).each(function(){
					...
					setTimeout(function(){
						...
						$.getScript(quickview_param.wc_quantity_increment);
					}, 1000);
				});
